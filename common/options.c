...
#ifndef NO_MODELINE
# ifdef SYSV_COMPAT
	{ "modelines",	"ml",	BOOL,	CANSET,		o_modeline	},
# else
...
void initopts()
{
	...
	if ((val = getenv("COLUMNS")) && atoi(val) > 4) /* yes, ASSIGNMENT! */
	{
		COLS = atoi(val);
	}
#endif
	...
	char	*assignments;	/* a string containing option assignments */
{
	...
	char	*prefix;	/* pointer to "~" or "no" at front of a boolean */
	int	i, j;
	...
	for (name = assignments; *name; )
	{
		...
		if (*scan == '=')
		{
			*scan++ = '\0';
			...
			{
				value = scan;
				while (*scan && (*scan != ' ' && *scan != '\t' || QTST(scan)))
				{
					scan++;
				}
				if (*scan)
