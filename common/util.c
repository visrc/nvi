...
#ifndef lint
static char sccsid[] = "$Id: util.c,v 5.53 1993/05/20 20:30:21 bostic Exp $ (Berkeley) $Date: 1993/05/20 20:30:21 $";
#endif /* not lint */
 ...
 *	and "mkexrc" have been added to the list.  Where it gets interesting
 *	is that the alternate file name is set multiple times by a commands.
 *	If an edit attempt fails (for whatever reason, like the current file
 *	is modified but as yet unwritten), it is set to the file name that
 *	the user didn't get to edit.  If the edit succeeds, it is set to the
 *	last file name that was edited.  Good fun.
 *
